...
def default_argument_parser():
    ...
    out_dir = make_out_dir(output_dir, model_name)
...
def make_out_dir(output_dir, model_name):
    def find(_out_dir, _model_name, num):
        path = os.path.join(_out_dir, _model_name+'({})'.format(num))
        if not os.path.exists(path):
            return path
        file = os.path.join(path, 'model_0009999.pth')
        if not os.path.exists(file):
            shutil.rmtree(path)
            return path
        return find(_out_dir, _model_name, num+1)

    out_dir = os.path.join(output_dir, model_name)
    if os.path.exists(out_dir):
        out_dir = find(output_dir, model_name, 1)
    # os.mkdir(out_dir)
    return out_dir
